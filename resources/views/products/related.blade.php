            ...
            <div class="product-thumb">
                ...
                @if(is_null($user_id))
                    <a  href="{{ route('login') }}" class="favor_btn favor_btn_{{ $post->id }}" data-id="{{ $post->id }}" data-user="{{ $user_id }}">
                        <img src="{{asset('assets/heart.png')}}" alt="" style="width: 25px;height: 25px">
                    </a>
                @else
                    <a  href="javascript:void(0);" class= "favor_btn favor_btn_{{ $post->id }}" data-id="{{ $post->id }}" data-user="{{ $user_id }}">
                        @if(in_array($post->id,$favorites))
                            <img src="{{asset('assets/heart-o.png')}}" alt="" style="width: 25px;height: 25px">
                        @else
                            <img src="{{asset('assets/heart.png')}}" alt="" style="width: 25px;height: 25px">
                        @endif
...
{{--                                                <span>{{App\City::($post->city_id)->city}}</span>--}}
                        <span>{{substr($post->location,0,20)}}</span>
